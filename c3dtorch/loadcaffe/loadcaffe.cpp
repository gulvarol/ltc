...
void convertProtoToLuaV1(const caffe::NetParameter &netparam, const char* lua_name, const char* cuda_package)
{
  ...
  for (int i=0; i<netparam.layers_size(); ++i)
  {
    ...
    switch(layer.type())
    {
      ...
      //case caffe::V1LayerParameter::CONVOLUTION3D:
      case 33:
      {
        auto &param = layer.convolution_param();
        int groups = param.group() == 0 ? 1 : param.group();
        ...
        //int nOutputPlane = param.num_output();
        num_output = nOutputPlane;
        int kT,kW,kH,dT,dW,dH,pad_t,pad_w,pad_h;
        kW = param.kernel_size();
        kH = kW;
        kT = param.kernel_depth();
        dW = param.stride();
        dH = dW;
        dT = param.temporal_stride();
        pad_w = param.pad();
        pad_h = pad_w;
        pad_t = param.temporal_pad();
        char buf[1024];
        sprintf(buf, "nn.VolumetricConvolution(%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)",
            nInputPlane, nOutputPlane, kT, kW, kH, dT, dW, dH, pad_t, pad_w, pad_w);
        lines.emplace_back(layer.name(), buf);
        break;
      }
      /* case caffe::V1LayerParameter::POOLING3D: */
      case 0:
      {
        ...
        if(param.pool() == caffe::PoolingParameter::MAX)
          sprintf(buf, "nn.VolumetricMaxPooling(%d, %d, %d, %d, %d, %d, %d, %d, %d):ceil()", kT, kH, kW, dT, dH, dW, pad_t, pad_h, pad_w);
...
void loadModuleV1(const caffe::NetParameter* netparam, const char* name, THFloatTensor* weight, THFloatTensor* bias)
{
  ...
  for(int i=0; i<n; ++i)
  {
    ...
    if(std::string(name) == layer.name())
    {
      ...
      if(layer.type() == 33)
      {
        THFloatTensor_resize5d(weight, nOutputPlane, nInputPlane, kW, kH, kW);
        memcpy(THFloatTensor_data(weight), layer.blobs(0).data().data(), sizeof(float)*nOutputPlane*nInputPlane*kW*kH*kW);
      }
      else
      {
